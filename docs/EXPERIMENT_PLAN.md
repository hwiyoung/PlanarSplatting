...
**데이터 준비 시점**: Phase 3-B ablation 학습 중(GPU 유휴 시간) UrbanScene3D / Building3D 다운로드 및 COLMAP 전처리 선행 가능. 비교 방법 및 평가 지표 설계는 Phase 3-B 결과 확인 후 결정 — L_mutual 효과가 가장 크게 나타난 지표(Normal cos 또는 mIoU)를 중심으로 비교 방법을 선택해야 실험 설계가 연구 기여와 정렬됨.
...
**사후 보완 (Phase 4 진입 전 수행):** SAM instance mask 보존
- `generate_segmentation.py`에 `--save_instance_maps` 플래그 추가
- Grounded SAM이 검출한 per-building mask를 `instance_maps/`에 별도 저장 (0=bg, 1=building_1, 2=building_2, ...)
- 기존 seg_maps와 독립적이며, 학습 파이프라인에는 사용되지 않음
- Phase 4의 building grouping 보조 자료로 활용 (SAM instance vote)
...
| 검증 항목 | Go | Retry |
|-----------|-----|-------|
| Gradient check | ∂L_mutual/∂f_i ≠ 0, ∂L_mutual/∂R_i ≠ 0 (양방향 동시) | 실패 → detach 여부, e_gravity 방향 확인 |
| mutual_mode 분리 | sem2geo: R_i만 non-zero, geo2sem: f_i만 non-zero | 불일치 → detach 로직 점검 |
| Smoke test | L_mutual 감소 추세 (증가/NaN 아님). Gradient check(#1)이 "gradient 존재"를 검증한다면, smoke test는 "그 gradient로 optimizer.step() 시 실제 loss 감소"를 검증 | 문제 → λ_m, warmup 파라미터 점검 |
| 기존 기능 보존 | lambda_mutual=0 시 L_mutual 코드 경로 미진입 확인 (조건문 분리). 크래시/NaN 없음 | 간섭 → 플래그 분기 점검 |
...
1. L_mutual (RESEARCH_CONTEXT.md 수식 참조)
   - 3항 구조: p_wall·L_vert + p_roof·L_slope + p_ground·L_horiz
   - L_slope = relu(τ − (n·e_z)²)², τ=0.15: 단측 wall exclusion (roof로 분류된 프리미티브가 벽처럼 수평인 경우만 penalty. flat/gable/mansard 등 정상적 지붕 경사에는 penalty 0)
   - e_z(중력 방향): COLMAP world frame에서 gravity ≈ -Y → 코드에서 e_gravity = [0, -1, 0] 사용
...
6. utils_demo/에 ablation .conf 7개 생성 (pyhocon, base config에 merge 가능하도록, 기존 phase2c.conf와 동일 디렉토리)
   - RESEARCH_CONTEXT.md Ablation 설계 참조: core (a)~(d), directional (e)~(f), warmup (g)
7. 패키지 필요하면 Dockerfile에도 추가

results/phase3a/REPORT.md 작성 (EXPERIMENT_PLAN.md 하단 REPORT 템플릿에 따라 작성. 구현 Phase이므로 정량 지표는 gradient check/smoke test 결과로 대체):
...
- Warmup Ablation (g): warmup 필요성 검증

**Go/No-Go:**
- **Go**: Joint(d)가 Independent(c) 대비 Normal cos **또는** mIoU 중 하나 이상에서 개선, 나머지 지표 유의미한 악화 없음.
  - Normal cos: ∂L_mutual/∂R_i 직접 효과 (의미론→기하 방향)
  - mIoU: ∂L_mutual/∂f_i 직접 효과 (기하→의미론 방향)
  - Depth MAE는 L_mutual 직접 관여 지표가 아닌 보조 참고 (c_i가 L_mutual 수식에 없음)
- **핵심 증거는 Directional Ablation**: (d)가 단방향 (e),(f) 각각보다 우수 → 양방향 시너지 입증
...
Warmup Ablation:
- (g) no_warmup: (d)에서 warmup 없이 λ_m 즉시 적용

utils_demo/에 ablation .conf 7개 생성 (pyhocon, base config에 merge 가능하도록):
- ablation_a_geo_only.conf ~ ablation_g_no_warmup.conf
- **중요**: (c) independent의 config는 Phase 2-C의 phase2c.conf와 동일해야 한다 (동일 hyperparameter set). 다른 조건들은 이 base에서 해당 요소만 변경한다.
...
results/phase3b/REPORT.md 작성 (EXPERIMENT_PLAN.md 하단 REPORT 템플릿에 따라 작성):
- "수행 작업 요약": Phase 3-B가 전체 연구 흐름에서 어디에 위치하는지, 무엇을 검증하려는지, 양방향 상호 보강 가설에 어떻게 기여하는지 서술
- "정량 지표": 7조건(a)~(g) × 3지표(Depth MAE, Normal cos, mIoU) 비교 표 (delta vs (c) 포함)
  - Normal cos, mIoU: L_mutual 직접 효과 지표 — 변화의 원인을 해석
  - Depth MAE: 보조 참고 (c_i가 L_mutual 수식에 없어 간접 효과)
- "핵심 해석" (논문 클레임 지지 논증):
  - (d)-(c) delta: L_mutual 전체 효과
  - (d)-(e) delta, Normal cos: "의미론→기하" 방향 기여 크기
  - (d)-(f) delta, mIoU: "기하→의미론" 방향 기여 크기
  - (d) > max((e),(f)): 양방향 시너지 입증 여부 — 논문 핵심 클레임
  - (g)-(d) delta: warmup 필요성
- "정성적 결과": 조건별 렌더링 비교 이미지 (8~9장), 건물 유형/지붕 형태/카메라 각도 다양 선별
- "Go/No-Go": 기준 충족 여부 + Phase 3-C 또는 Phase 4 진행의 전제조건 만족 논증
results/SUMMARY.md 생성: Phase 0~3-B 핵심 지표를 Phase별+ablation 조건별로 요약하는 표. EXPERIMENT_PLAN.md 하단 SUMMARY.md 섹션 참조.
CLAUDE.md 진행 상태 업데이트.
```
...
**포함 기준**: 학위논문 — (h)~(i) 포함 (추가 실험 챕터). 학회 논문 — core ablation (a)~(g)가 주 실험, (h)~(i)는 supplementary 또는 생략. Phase 3-B 완료 후 논문 투고 계획에 따라 결정.
...
| **(h)** photo_independent | 추가 | 없음 (λ_m=0) | vs Phase 2-C (L_photo 없음, L_mutual 없음) |
| **(i)** photo_joint | 추가 | Full | vs (h) 및 Phase 3-B(d) |

**확인 사항:**
- (h) vs Phase 2-C: L_photo 추가만으로 기하 지표(Depth MAE, Normal Error)가 개선되는가?
- (i) vs (h): L_photo가 있을 때도 L_mutual이 추가 개선을 주는가?
- (i) vs Phase 3-B (d): L_photo 추가가 전체적으로 더 나은가?
...
- (h) photo_independent: --enable_photo --lambda_photo 1.0 --lambda_geo 0.1 (L_mutual 없음)
- (i) photo_joint: --enable_photo --lambda_photo 1.0 --lambda_geo 0.1 --mutual_mode full

각각 evaluate.py 실행. Phase 3-B 결과와 비교.
results/phase3c/REPORT.md 작성 (EXPERIMENT_PLAN.md 하단 REPORT 템플릿에 따라 작성. (h)/(i) 비교 표 + 이미지).
results/SUMMARY.md 업데이트: (h)/(i) 결과 추가.
CLAUDE.md 진행 상태 업데이트.
```

---

## Phase 4: Building Grouping + CityGML LOD2 변환 + 검증

**목표:** CUDA rasterizer에 primitive ID 채널 추가, L_planar/L_adj 구현, building instance grouping, 프리미티브 → CityGML LOD2 + val3dity 검증.

**선행 요건:** Phase 2-A instance_maps 생성 완료 (사후 보완 항목 참조)

**프롬프트:**
```
docs/EXPERIMENT_PLAN.md의 Phase 4를 진행해줘. 컨테이너 내부에서 작업이야.
docs/RESEARCH_CONTEXT.md의 "L_geo" 섹션과 "Building Instance Segmentation" 섹션도 참고해줘.
docs/BUILDING_INSTANCE_ANALYSIS.md도 참고해줘.
...
Part 2: Building Grouping (전략 2: 기하학 기반 + SAM 보조)
scripts/building_grouping.py를 만들어줘.
1. 기하학 기반 grouping:
   - argmax(softmax(f_i)) → roof/wall/ground 분류
   - Roof 프리미티브의 중심 좌표를 수평면(XZ, COLMAP gravity=-Y)에 투영
   - Distance threshold(= max(r_i) × 3) 기반 connected component → building_id 부여
2. Wall 귀속: 각 wall 프리미티브의 수평 거리 기준 최근접 roof cluster에 할당
3. SAM instance vote 보강:
   - Phase 2-A의 instance_maps/ 참조
   - Primitive center를 각 view에 투영 → instance_map lookup → multi-view majority vote
   - 기하학 기반으로 하나의 cluster에 속하지만 SAM이 다른 building으로 검출한 경우 분리 검토
4. GroundSurface: 각 building의 roof boundary를 지면에 투영
5. 한계 사항: 동일 높이 인접 건물, 벽 공유 연립주택에서 under-segmentation 가능 → REPORT에 명시

Part 3: CityGML LOD2 변환 + val3dity 검증
scripts/export_citygml.py를 만들어줘.
먼저 Phase 3 checkpoint 구조를 파악해줘.

처리 (per-building):
1. 같은 클래스 + cos(normal) > 0.95 + 중심 거리 < 2*r_i → 프리미티브 병합
2. Per-building alpha shape → 경계 다각형
3. CityGML XML 생성:
...
results/phase4/REPORT.md 작성 (EXPERIMENT_PLAN.md 하단 REPORT 템플릿에 따라 작성).
results/SUMMARY.md 최종 업데이트: Phase 4 결과(val3dity, CityGML) 추가. 전체 파이프라인 완성 요약.
CLAUDE.md 진행 상태 업데이트.
```
...
## 수행 작업 요약
<!-- 작성 지침:
1. 이 Phase가 전체 연구 흐름에서 어디에 위치하는지를 1문단으로 서술할 것.
2. 이전 Phase에서 확보한 전제조건이 무엇인지 명시.
3. 이 Phase에서 검증하려는 것이 무엇인지 명시.
4. 이것이 핵심 가설(L_mutual의 양방향 상호 보강)의 검증에 어떻게 기여하는지 서술.
   예: "Phase 2-C에서 gradient 격리가 확인되었으므로, Phase 3-A에서 L_mutual 활성화 시
   기하학적 변화를 L_mutual에 귀속시킬 수 있다." -->
(무엇을 했는지 간결하게, 연구 맥락 포함)

## 정량 지표
<!-- 작성 지침:
1. Phase 간 비교 시 GT 기준이 다르면 반드시 명시 (예: Phase 0은 Metric3D 대비, Phase 1+는 MVS 대비).
2. 단순 수치 나열이 아니라, 변화의 원인을 1~2문장으로 해석할 것.
3. 통계적 유의성이 있다면 std도 포함. -->
| 지표 | 값 | 이전 Phase | 변화 | GT 기준 |
|------|-----|-----------|------|---------|
| Depth MAE | x.xxxx | x.xxxx | -x.xxxx | (MVS/Metric3D) |
| Normal cos | x.xxxx | x.xxxx | +x.xxxx | (MVS/Metric3D) |
| mIoU | x.xx (Phase 2+) | x.xx | +x.xx | seg_maps v11 |
| PSNR | xx.x dB (Phase 3-C only) | — | — | GT RGB |

## 정성적 결과
<!-- 작성 지침:
1. 데이터셋 전체에서 다양한 장면을 선별 (초반/중반/후반 균등 분포, 8~9장 이상).
2. 선별 기준: 건물 규모 다양 (고층/저층), 지붕 형태 다양 (flat/경사), 카메라 각도 다양 (oblique/near-nadir).
3. 각 이미지에 구체적 캡션 포함:
   - 어떤 구조물(건물 유형, 지붕 형태, 카메라 각도)인지
   - 어떤 결과가 나타났는지 (정확한 분류/오분류, 기하학적 특징)
   - 이전 Phase와 달라진 점이 있으면 지적
4. 이미지는 results/phaseX/images/에 저장하고 상대 경로로 참조. -->
...
## Go/No-Go 판단
<!-- 작성 지침:
1. 숫자 기준 충족 여부뿐 아니라, 다음 Phase의 전제조건이 만족되었음을 논증할 것.
   예: "Normal cos가 Phase 1과 동등하므로, gradient 격리가 실험적으로 확인되었다.
   이는 Phase 3-A에서 L_mutual 활성화 시 기하학적 변화를 L_mutual에 귀속시킬 수 있는 근거이다."
2. 불합격 시 어떤 경로(Retry/Switch)를 택할지와 그 근거도 서술. -->
- [x] Go / [ ] Retry / [ ] Switch
- 근거: ...

## 생성/수정 파일 목록
<!-- 작성 지침:
1. 코드 변경의 학술적 의의가 있는 경우 그 의의를 한 문장으로 서술.
   예: "backward.cu color gradient 추가 — L_sem→f_i gradient path를 가능하게 하면서
   L_sem→geometry 격리를 유지하기 위한 의도적 설계."
2. 파일 목록은 변경 유형(신규/수정)과 핵심 변경 내용을 포함. -->
| 파일 | 유형 | 핵심 변경 |
|------|------|----------|
| example.py | 신규 | 설명 |
...
## 다음 Phase
(다음에 할 일, 이 Phase의 결과가 다음 Phase에 어떤 전제조건을 제공하는지)
```

※ 이미지는 `results/phaseX/images/` 에 저장하고 상대 경로로 참조.
※ 정성적 결과가 중요한 이유: 숫자만으로는 벽면/지붕 분리가 제대로 되는지 판단 불가. 시각적 확인이 필수.
※ 구현 Phase (2-B, 3-A 등): 정량 지표를 gradient check/smoke test 결과로 대체. 정성적 결과는 "해당 없음 (구현 Phase)"으로 표기.
...
요구사항:
1. 모든 수치의 GT 기준을 통일할 것 (MVS depth/normal 기준으로 통일. Phase 0의 Metric3D 기준 수치는 별도 행으로 표기)
2. 각 Phase의 학술적 의의를 한 줄씩 붙일 것 (예: "Phase 2-C: gradient 격리 실험적 확인 — L_mutual 귀속 근거 확보")
3. GT 기준이 다른 Phase 간 비교에는 "직접 비교 불가" 각주 표기
```