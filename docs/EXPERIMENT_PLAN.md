...
| Smoke test (5 iter) | L_mutual 감소 추세 (증가/NaN 아님) | 문제 → λ_m, warmup 파라미터 점검 |
| 기존 기능 보존 | --mutual_mode none에서 Phase 2-C와 동일 수치 | 간섭 → 플래그 분기 점검 |
...
results/phase3c/REPORT.md 작성 ((h)/(i) 비교 표 + 이미지).
CLAUDE.md 진행 상태 업데이트.
```
