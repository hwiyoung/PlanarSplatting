...
def resume_model(trainer, ckpt_name='latest'):
        ...
        # Backward compatibility: add missing keys for older checkpoints
        ...
        if color_key not in saved_model_state["model_state_dict"]:
            saved_model_state["model_state_dict"][color_key] = torch.full((plane_num, 3), 0.5)
            logger.info(f'Added missing {color_key} ({plane_num}, 3) for backward compatibility')
